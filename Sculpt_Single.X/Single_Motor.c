...
//        clear_LED();
   //  Pulse until there's no steps left
    if(turned_on && stp_2.stps_left > 0){
    //if (stp_1.stps_left > 0){
        mPORTBToggleBits(stp_2.STP);
        stp_2.stps_left--;      
    }
    if (stp_2.stps_left == 0) keep_moving = 0;
